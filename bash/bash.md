...
## 1. 变量
### 1.1 变量的命名
Shell的变量必须以字符或者下划线开头，后面可以跟数字、字母、下划线，变量的长度没有限制。  
...
但是需要注意以下几点：  
1. 变量名和变量值之间用等号相连，之间没有任何空格
2. 当变量值中有空格时必须用引号括起来，否则会出现错误。其中引号可以是单引号，也可是双引号。

变量的取值也十分简单，只需要在变量名前面加上$符号即可，严谨一点的写法是${}

大括号{}在Shell中的用法很多，最常见的用法就是引用变量名，又叫做变量扩展，常见用法如下：  

|表达式|作用|
|:----|:----|
|${VAR}|取出变量VAR的值|
|${VAR-DEFAULT}|如果VAR没有定义，则以$DEFAULT作为其值|
|${VAR:-DEFAULT}|如果VAR没有定义或者VAR的值为空，则以$DEFAULT作为其值|
...
|${VAR:+VALUE}|如果定义了VAR并且VAR不为空,则值为$VALUE,否则为空字符串|
|$VAR?MSG|如果VAR没有定义则打印MSG|
|${VAR:?MSG}|如果VAR没有定义或未被赋值，则打印MSG|
|${!PREFIX@} ${!PERFIX*}|匹配所有以PERFIX开头的变量
|${#STR}|返回变量${STR}的长度|
|$STR:POSITION|从位置$POSITION处提取子串|
|$STR:POSITION:LENGTH|从位置$POSITION处提取长度为$LENGTH的字符串|
...
### 1.3 特殊变量
Shell中有一些预先定义的特殊变量，他们的取值只有在脚本运行时才能确定。
#### 1.3.1 位置变量
位置变量的命名简单直接，比如，脚本本身是$0,第一个参数是$1,第二个参数是$2，第三个参数是$3,以此类推，当位置变量的个数大于9时，需要用${}进行标识，例如${10}

### 1.4 脚本或者命令的返回值
$?永远表示上一个命令的返回值。在Linux中规定正常退出的命令或脚本是以0作为返回值的，任何非0的返回值表示命令未正确退出或者未正常执行。

## 2. 引用
引用是指将字符串用某种符号括起来，以防止特殊字符变解析成其他意思。  
Shell中一共有4种引用，分别如下：  

1.双引号，又称为弱引用或者部分引用，可以引用$符、反引号、转义符之外的所有字符。  
2.单引号，又称为强引用或者全引用，可以引用所有的字符，单引号中间不能再有单引号，否则Shell无法判断到底哪里是单引号的起止位置。  
3.反引号，会将括起来的字符串届时为系统命令  
4.转义符，即'\'符号，用来引用单个字符
...
和  
    ...
    $(命令)
如果被引用的命令输出包含多行，此时若不通过引用的方式输出变量，则输出内容会被删除换行符，换行之间会使用系统默认空字符输出，即输出的内容为一行。
...
## 4. 算术扩展
算术扩展是Shell提供的整数变量的运算机制，是Shell的内建命令之一，语法如下  
